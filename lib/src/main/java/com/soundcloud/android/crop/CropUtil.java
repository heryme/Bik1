...
public class CropUtil {
    ...
    public static final String SUB_DIR = "/Biker/";
    ...
    public static File getOutputMediaFile() {
        ...
        File file = new File(APP_DIR + SUB_DIR + IMAGES_DIR);
        if (!file.exists())
            file.mkdirs();

        SimpleDateFormat formatter = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss");
        Date now = new Date();
        return new File(file.getPath() + File.separator + "BikerTemp" + formatter.format(now) + ".jpg");
    }
    ...
    public static File getImageFile() {
        ...
        android.util.Log.d("Files", "Path: " + path);
    ...
    public static void deleteImage() {
        ...
        if (files != null && files.length > 0) {
            ...
            for (int i = 0; i < files.length; i++) {
                android.util.Log.d("Test Files", "--->FileName:" + files[i].getName());
                files[i].getAbsoluteFile().delete();
                android.util.Log.d("getZipFile", "--->getZipFile:" + files[i].getAbsoluteFile());
            }
        }
    }
}
